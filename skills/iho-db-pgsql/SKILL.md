---
name: iho-db-pgsql
description: 用于按 IHO 项目风格设计、评审和整理 PostgreSQL 建表与 DDL 变更脚本。适用于 `com.cnhis.iho` 相关模块中的短码表建表、扩表、字段设计、索引设计、注释规范、JSONB/数组字段设计和 DDL 文档整理，参考样本包括 `ndy1`、`ndz1`、`nea1`。最终输出统一落到项目根目录 `doc/specs/ddl-design.md`。
---
...
本 skill 只关注 PostgreSQL 建表与 DDL 设计规范，不涉及 Java 实体、DTO、Mapper、Service 或其他应用层代码。以 [references/style-guide.md](references/style-guide.md) 作为建表风格依据，并将最终产出的建表脚本统一整理到项目根目录的 `doc/specs/ddl-design.md`。
...
8. 如果 `doc/specs` 不存在，先创建目录，再写入 `ddl-design.md`。

## 输出要求

- 最终交付物必须是 Markdown 文件，而不是只在对话里贴 SQL。
- 文件路径固定为 `doc/specs/ddl-design.md`。
- 文件内容建议至少包含：
  - 变更背景
  - 设计原则
  - 建表/改表脚本
...
- 常用过滤字段是否补了索引。
- 表注释和列注释是否完整。
- 最终脚本是否已经写入 `doc/specs/ddl-design.md`。

## 备注

- 当前仓库里可确认的样本为 `ndy1`、`ndz1`、`nea1`。如果需求里写的是 `ndyz1`，默认按“很可能是指 `ndz1`”处理，除非后续证据表明另有其表。
- 当前项目根目录下尚未存在 `doc/specs` 目录，因此实际输出时需要主动创建。
