...
1. 用户显式指令（`AGENTS.md`、直接要求）
2. Superpowers skills
3. 默认系统行为

如果 `AGENTS.md` 说“不要用 TDD”，而某个 skill 写着“始终使用 TDD”，听用户的。

## 在 Codex 中如何访问 skills

Codex 会原生发现 `~/.agents/skills/` 下的 skills，并按 `SKILL.md` frontmatter 决定是否加载。

做法是：

1. 先判断当前任务是否可能命中某个 skill
2. 一旦命中，就读取并遵循该 skill
3. 不要凭记忆假设 skill 内容没变

如需看工具映射，查看 `references/codex-tools.md`。
