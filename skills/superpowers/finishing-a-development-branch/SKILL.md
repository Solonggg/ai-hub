...
**额外约束：**本 skill 不允许为了收尾而直接执行新的 `git commit`。如果代码只是“写完了但还未提交”，先停在评审与用户确认阶段，不要擅自补提。
...
```text
当前存在未提交变更。本流程不允许在代码写完后直接 git commit 来推进收尾。

请先完成代码评审，并由用户明确决定是否执行后续 Git 动作；在此之前，不继续本 skill。
```
...
- `/doc/feat/feat_xxxx.md` 是否已补充
- SQL、参数、接口、资源、版本依赖等变更是否已同步写入
...
- 新需求进入收尾前先确认 `/doc/feat/feat_xxxx.md` 已更新
- 先验证测试，再给选项
- 始终给出固定 4 选项
- 丢弃前必须拿到精确确认
- 仅在需要时清理 worktree
